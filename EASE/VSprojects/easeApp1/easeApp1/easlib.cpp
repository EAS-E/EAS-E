...
// create 'the system' structure for the the library - easlib_sys*
easlib_sys* easlib;
void easlib_init() {
	easlib = (easlib_sys*)E_CREATE_F(0, 2, sizeof(easlib_sys));
}


typeInfo* f_typeInfos;

// delete a persitent instance reference
...
void WTI_R(int value, int width) {	// write as I n
	...
	if (width <= 0)
		return;
	if (uib_w->col + width >= uib_buffsize)
		return;
	...
	if ((rem > 0) | (sign & (i < 0)))
		...
		if (sign)
			uib_w->buffer[uib_w->col + i] = minus;
	}
...
void WDD_R(double val, int width, int decimal) {
	...
	w = width - decimal - 1;
	if (i != 0)
		WTI_R(i, w);	// write non-zero integer part (fraction only will drop any minus sign)
	else {
		w--;
		if (minus)
			w--;
	...
	uib_w->buffer[uib_w->col] = point;	// decimal point
...
Etxt* RDT_F(int width) {
	...
	if (uib_r->col == 0)
		return NULL;
	if ((uib_r->rlen - uib_r->col) < width - 1)		// col starts at 1; buffer is 0-based
		return NULL;
...
int RDN_F() {				// free-form read an integer
	...
	while (uib_r->col <= uib_r->rlen) {
		if (uib_r->buffer[uib_r->col - 1] != *_T(" ")) {			// found a non-space
			if (uib_r->buffer[uib_r->col - 1] >= *_T("0") & uib_r->buffer[uib_r->col - 1] <= *_T("9")) {	// a digit?
				digit = uib_r->buffer[uib_r->col - 1];
				value = value * 10 + digit - zero;
			}
			else {
				if (sign == 0 & value == 0) {	// nothing but spaces/zeroes, so far...
					if (uib_r->buffer[uib_r->col - 1] == *_T("-"))
						sign = 1;
					if (uib_r->buffer[uib_r->col - 1] == *_T("+"))
						sign = 2;			// we've seen a sign - so don't allow another (but plus means nothing)
				}
				else
					break;	// anything else, we are done with integer part
			}
			uib_r->col++;
			if (uib_r->col > uib_r->rlen) {
				uib_r->rlen = 0;
			...
			if ( digit + sign != 0)				// working on a value, found a space ? - we're done
				break;
			uib_r->col++;
			if (uib_r->col > uib_r->rlen) {		// all blanks to end of line?
				uib_r->rlen = 0;
				while (uib_r->rlen == 0) {
					RDL_R();
					if (uib_r->eof == 2)
...
double RDD_F(int width, int decimal) {
	...
	if (width <= 0)
		return 0;
	if (uib_r->col + width >= uib_r->rlen)
		return 0;

	i = uib_r->col;
	val = RDI_F(width - decimal - 1);
	i = uib_r->col - i;
	decimal = width - i - 1;

	if (uib_r->buffer[uib_r->col - 1] == point) {
		uib_r->col++;
		while (decimal > 0) {
			decimal--;
			i = uib_r->buffer[uib_r->col - 1];
			if (i < zero | i > zero + 9)
				break;
			d = d * 10;
			frac = i - zero;
...
double RDU_F() {			// unformatted read real
	...
	while (uib_r->col <= uib_r->rlen) {
		if (uib_r->buffer[uib_r->col - 1] != *_T(" ")) {			// found a non-space
			...
			if (value != 0)				// working on a value, found a space ? - we're done
				break;
			uib_r->col++;
			if (uib_r->col > uib_r->rlen) {		// all blanks to end of line?
				uib_r->rlen = 0;
	...
	if (uib_r->buffer[uib_r->col - 1] == point) {	// there may be fractional part
		uib_r->col++;
		while ( uib_r->col <= uib_r->rlen) {
			digit = uib_r->buffer[uib_r->col - 1];
			if (digit < zero | digit > zero + 9)
